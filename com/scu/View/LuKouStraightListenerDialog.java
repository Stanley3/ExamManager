 ...
 *
 */
public class LuKouStraightListenerDialog implements ActionListener{
	...
	//主窗口
	private static JFrame LukouFrm;
	//是否开启此项评判复选框
	...
	public void actionPerformed(ActionEvent e) {
		...
		LukouFrm.setSize(WIDTH, HEIGHT);
		LukouFrm.setUndecorated(true);
		LukouFrm.setContentPane(contentPanel);
		LukouFrm.setLocation(x, y);
		LukouFrm.getRootPane().setWindowDecorationStyle(JRootPane.PLAIN_DIALOG);
		
		isOpenJudge = new JCheckBox("开启此项评判", ConfigManager.luKouStraight.isOpen());
		...
		td.add(timeMode);
		td.add(distanceMode);
		if(ConfigManager.luKouStraight.getTimeOrDistance() == 1){
		...
		endTime = new JTextField(new Long(ConfigManager.luKouStraight.getEndTime() / 1000).toString(),4);
		endTime.setHorizontalAlignment(JTextField.CENTER);
		
		j_endDistance = new JLabel("结束距离 ", JLabel.CENTER);
		endDistance = new JTextField(new Integer(ConfigManager.luKouStraight.getEndDistance()).toString(),4);
		endDistance.setHorizontalAlignment(JTextField.CENTER);
		
		j_triggerDistance = new JLabel("触发距离 ", JLabel.CENTER);
		triggerDistance = new JTextField(new Integer(ConfigManager.luKouStraight.getTriggerDistance()).toString(),4);
		triggerDistance.setHorizontalAlignment(JTextField.CENTER);
		
		j_maxSpeed = new JLabel("最高车速 ", JLabel.CENTER);
		maxSpeed = new JTextField(new Integer(ConfigManager.luKouStraight.getMaxSpeed()).toString(),4);
		...
		ok.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				...
				ConfigManager.luKouStraight.setOpen(isOpen);
				
				boolean timeOrDistance = timeMode.isSelected();
				if(timeOrDistance){
					ConfigManager.luKouStraight.setTimeOrDistance(1);
				}else
				{
					ConfigManager.luKouStraight.setTimeOrDistance(0);
				}
				...
				}else
				{
					...
					ConfigManager.luKouStraight.setEndTime(endTimeValue);
				}
				...
				if(StringUtil.isStringEmpty(endDistanceString)){
					...
					ConfigManager.luKouStraight.setEndDistance(endDistanceValue);
				}
				...
				if (StringUtil.isStringEmpty(triggerDistanceString)) {
					...
					ConfigManager.luKouStraight.setTriggerDistance(triggerDistanceValue);
				}
				...
				if (StringUtil.isStringEmpty(maxSpeedString)) {
					...
					ConfigManager.luKouStraight.setMaxSpeed(maxSpeedValue);
				} 
