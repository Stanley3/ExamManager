 ...
 *
 */
public class TurnLeftListenerDialog implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		isOpenJudge = new JCheckBox("开启此项评判", ConfigManager.turnLeft.isOpen());
		...
		td.add(timeMode);
		td.add(distanceMode);
		if(ConfigManager.turnLeft.getTimeOrDistance() == 1){
		...
		endTime = new JTextField(new Long(ConfigManager.turnLeft.getEndTime() / 1000).toString(),4);
		endTime.setHorizontalAlignment(JTextField.CENTER);
		
		j_endDistance = new JLabel("结束距离 ", JLabel.CENTER);
		endDistance = new JTextField(new Integer(ConfigManager.turnLeft.getEndDistance()).toString(),4);
		endDistance.setHorizontalAlignment(JTextField.CENTER);
		
		j_triggerDistance = new JLabel("触发距离 ", JLabel.CENTER);
		triggerDistance = new JTextField(new Integer(ConfigManager.turnLeft.getTriggerDistance()).toString(),4);
		triggerDistance.setHorizontalAlignment(JTextField.CENTER);
		
		j_maxSpeed = new JLabel("最高车速 ", JLabel.CENTER);
		maxSpeed = new JTextField(new Integer(ConfigManager.turnLeft.getMaxSpeed()).toString(),4);
		maxSpeed.setHorizontalAlignment(JTextField.CENTER);
		
		j_minAngle = new JLabel("最小偏转角度", JLabel.CENTER);
		minAngle = new JTextField(new Integer(ConfigManager.turnLeft.getMinAngle()).toString(),4);
		...
		ok.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				...
				ConfigManager.turnLeft.setOpen(isOpen);
				
				boolean timeOrDistance = timeMode.isSelected();
				if(timeOrDistance){
					ConfigManager.turnLeft.setTimeOrDistance(1);
				}else
				{
					ConfigManager.turnLeft.setTimeOrDistance(0);
				}
				...
				}else
				{
					...
					ConfigManager.turnLeft.setEndTime(endTimeValue);
				}
				...
				if(StringUtil.isStringEmpty(endDistanceString)){
					...
					ConfigManager.turnLeft.setEndDistance(endDistanceValue);
				}
				...
				if (StringUtil.isStringEmpty(triggerDistanceString)) {
					...
					ConfigManager.turnLeft.setTriggerDistance(triggerDistanceValue);
				}
				...
				if (StringUtil.isStringEmpty(maxSpeedString)) {
					...
					ConfigManager.turnLeft.setMaxSpeed(maxSpeedValue);
				} 
				...
				if (StringUtil.isStringEmpty(minAngleString)) {
					...
					ConfigManager.turnLeft.setMinAngle(minAngleValue);
				} 
				TurnFrm.dispose();
			}
		});
