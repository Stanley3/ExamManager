 ...
 *
 */
public class MeetingCarListenerDialog implements ActionListener{
	...
	//方向盘度数
	private JLabel j_circleAngle;
	private JTextField circleAngle;
	...
	public void actionPerformed(ActionEvent e) {
		...
		MeetFrm.setSize(WIDTH, HEIGHT);
		MeetFrm.setUndecorated(true);
		MeetFrm.setContentPane(contentPanel);
		MeetFrm.setLocation(x, y);
		MeetFrm.getRootPane().setWindowDecorationStyle(JRootPane.PLAIN_DIALOG);
		
		isOpenJudge = new JCheckBox("开启此项评判", ConfigManager.meetingCar.isOpen());
		...
		td.add(timeMode);
		td.add(distanceMode);
		if(ConfigManager.meetingCar.getTimeOrDistance() == 1){
		...
		endTime = new JTextField(new Long(ConfigManager.meetingCar.getEndTime() /1000).toString(),4);
		endTime.setHorizontalAlignment(JTextField.CENTER);
		
		j_endDistance = new JLabel("结束距离 ", JLabel.CENTER);
		endDistance = new JTextField(new Integer(ConfigManager.meetingCar.getEndDistance()).toString(),4);
		endDistance.setHorizontalAlignment(JTextField.CENTER);
		
		j_triggerDistance = new JLabel("触发距离 ", JLabel.CENTER);
		triggerDistance = new JTextField(new Integer(ConfigManager.meetingCar.getTriggerDistance()).toString(),4);
		triggerDistance.setHorizontalAlignment(JTextField.CENTER);
		
		j_maxSpeed = new JLabel("最高车速 ", JLabel.CENTER);
		maxSpeed = new JTextField(new Integer(ConfigManager.meetingCar.getMaxSpeed()).toString(),4);
		maxSpeed.setHorizontalAlignment(JTextField.CENTER);
		
		j_circleAngle = new JLabel("方向盘最大角度 ", JLabel.CENTER);
		circleAngle = new JTextField(new Integer(ConfigManager.meetingCar.getCircleAngle()).toString(),4);
		circleAngle.setHorizontalAlignment(JTextField.CENTER);
		...
		panel6.add(j_circleAngle);
		panel6.add(circleAngle);
		panel6.add(angle);
		...
		ok.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				...
				ConfigManager.meetingCar.setOpen(isOpen);
				
				boolean timeOrDistance = timeMode.isSelected();
				if(timeOrDistance){
					ConfigManager.meetingCar.setTimeOrDistance(1);
				}else
				{
					ConfigManager.meetingCar.setTimeOrDistance(0);
				}
				...
				}else
				{
					...
					ConfigManager.meetingCar.setEndTime(endTimeValue);
				}
				...
				if(StringUtil.isStringEmpty(endDistanceString)){
					...
					ConfigManager.meetingCar.setEndDistance(endDistanceValue);
				}
				...
				if (StringUtil.isStringEmpty(triggerDistanceString)) {
					...
					ConfigManager.meetingCar.setTriggerDistance(triggerDistanceValue);
				}
				...
				if (StringUtil.isStringEmpty(maxSpeedString)) {
					...
					ConfigManager.meetingCar.setMaxSpeed(maxSpeedValue);
				} 
				
				String circleAngleString = circleAngle.getText();
				if (StringUtil.isStringEmpty(circleAngleString)) {
					JOptionPane.showMessageDialog(null, "方向盘角度不能为空！");
					return;
				} else {
					int circleAngleValue = Integer.parseInt(circleAngleString);
					ConfigManager.meetingCar.setCircleAngle(circleAngleValue);
				} 
				MeetFrm.dispose();
			}
		});
		
		MeetFrm.setVisible(true);
	}
}
