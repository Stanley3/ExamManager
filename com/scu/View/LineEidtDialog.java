 ...
 *
 */
public class LineEidtDialog extends JDialog {
	...
	public LineEidtDialog() {
		...
		this.list.addListSelectionListener(new ListSelectionListener() {
			public void valueChanged(ListSelectionEvent e) {
				int[] index = LineEidtDialog.this.list.getSelectedIndices();
				if ((index != null) && (index.length > 0)) {
					LineEidtDialog.this.btn_edit.setEnabled(true);
					LineEidtDialog.this.btn_del.setEnabled(true);
				}
			}
		});
		...
		this.btn_edit.setRolloverIcon(ConfigManager.signalIcon
				.getEdit_touchIcon());
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				LineEidtDialog.this.closeActionPerformed(e);
			}
		});
		...
		this.btn_del.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int selected = LineEidtDialog.this.list.getSelectedIndex();
				LineEidtDialog.this
						.delLine(LineEidtDialog.this.lineids[selected]);
				LineEidtDialog.this.refreshList();
				LineEidtDialog.this.list.updateUI();
				LineEidtDialog.this.btn_edit.setEnabled(false);
				LineEidtDialog.this.btn_del.setEnabled(false);
			}
		});
		this.btn_del.setRolloverIcon(ConfigManager.signalIcon
				.getDel_touchIcon());
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				EventQueue.invokeLater(new Runnable() {
					public void run() {
						new LineEditFrm(0).setVisible(true);
					}
				});
			}
		});
		...
		this.btn_edit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				LineEidtDialog.this.closeActionPerformed(arg0);
				int selected = LineEidtDialog.this.list.getSelectedIndex();
				LineEidtDialog.this.select = selected;
				EventQueue.invokeLater(new Runnable() {
					public void run() {
						new LineEditFrm(
								LineEidtDialog.this.lineids[LineEidtDialog.this.select])
								.setVisible(true);
					}
				});
			}
