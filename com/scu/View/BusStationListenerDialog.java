 ...
 *
 */
public class BusStationListenerDialog implements ActionListener{
	private static final long serialVersionUID = 1L;
	private static ImageIconSet allImage = new ImageIconSet();
	private static final int WIDTH = 500;
	private static final int HEIGHT = 450;
	//主窗口
	private static JFrame BusFrm;
	//是否开启此项评判复选框
	...
	public void actionPerformed(ActionEvent e) {
		...
		BusFrm.setSize(WIDTH, HEIGHT);
		BusFrm.setUndecorated(true);
		BusFrm.setContentPane(contentPanel);
		BusFrm.setLocation(x, y);
		BusFrm.getRootPane().setWindowDecorationStyle(JRootPane.PLAIN_DIALOG);
		
		isOpenJudge = new JCheckBox("开启此项评判", ConfigManager.busStation.isOpen());
		...
		td.add(timeMode);
		td.add(distanceMode);
		if(ConfigManager.busStation.getTimeOrDistance() == 1){
		...
		endTime = new JTextField(new Long(ConfigManager.busStation.getEndTime() / 1000).toString(),4);
		endTime.setHorizontalAlignment(JTextField.CENTER);
		
		j_endDistance = new JLabel("结束距离 ", JLabel.CENTER);
		endDistance = new JTextField(new Integer(ConfigManager.busStation.getEndDistance()).toString(),4);
		endDistance.setHorizontalAlignment(JTextField.CENTER);
		
		j_triggerDistance = new JLabel("触发距离 ", JLabel.CENTER);
		triggerDistance = new JTextField(new Integer(ConfigManager.busStation.getTriggerDistance()).toString(),4);
		triggerDistance.setHorizontalAlignment(JTextField.CENTER);
		
		j_maxSpeed = new JLabel("最高车速 ", JLabel.CENTER);
		maxSpeed = new JTextField(new Integer(ConfigManager.busStation.getMaxSpeed()).toString(),4);
		...
		ok.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				...
				ConfigManager.busStation.setOpen(isOpen);
				
				boolean timeOrDistance = timeMode.isSelected();
				if(timeOrDistance){
					ConfigManager.busStation.setTimeOrDistance(1);
				}else
				{
					ConfigManager.busStation.setTimeOrDistance(0);
				}
				...
				}else
				{
					...
					ConfigManager.busStation.setEndTime(endTimeValue);
				}
				...
				if(StringUtil.isStringEmpty(endDistanceString)){
					...
					ConfigManager.busStation.setEndDistance(endDistanceValue);
				}
				...
				if (StringUtil.isStringEmpty(triggerDistanceString)) {
					...
					ConfigManager.busStation.setTriggerDistance(triggerDistanceValue);
				}
				...
				if (StringUtil.isStringEmpty(maxSpeedString)) {
					...
					ConfigManager.busStation.setMaxSpeed(maxSpeedValue);
				} 
